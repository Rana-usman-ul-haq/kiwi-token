...
Twitter: http://x.com/KiwiSwapX
Telegram: http://t.me/KiwiSwaps
Website: http://kiwiswaps.com/
Documentation: http://docs.kiwiswaps.com/
...
contract Kiwi is Context, IERC20, Ownable {
    ...
    bool public kiwiBartering = false;
    address payable public _seedBank = payable(0x9D15Da5F7650b37f17152aaf662C5f5406566BdC);
    ...
    constructor () {
      ...
      //  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // test router address
        ...
        _isExcludedFromFee[_seedBank] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            require(kiwiBartering, "Kiwi trading not opened");

            if(to != uniswapV2Pair){
               require(balanceOf(to) + amount <= maxKiwiLimit, "Cannot keep more Kiwis than permitted");
            }

            if(_juicyBuyFee > 0) {
            if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                taxAmount = amount.mul(_juicyBuyFee).div(100);
            }
            }

            if(_juicySellFee > 0) {
            if(to == uniswapV2Pair){
                taxAmount = amount.mul(_juicySellFee).div(100);
            }
            }
    ...
    function sendETHToFee(uint256 amount) private {
        (bool callSuccess, ) = payable(_seedBank).call{value: amount}("");

        if (!callSuccess) {
        // Log the failure but do not revert the transaction
        emit TaxWalletPaymentRevert(_seedBank, amount);
    }
    ...
    function updateFruityTaxes(uint256 buyFee, uint256 sellFee) public onlyOwner {
        _juicyBuyFee = buyFee;
        _juicySellFee = sellFee;
    }

    function manualSwap() external {
        require(_msgSender()==_seedBank);
    ...
    function enableKiwiTrade() public onlyOwner {
        require(kiwiBartering != true, "Kiwi Trade enabled already");
        kiwiBartering = true;
    }
    ...
    function modifyKiwiWalletLimit(uint256 _maxLimit) public onlyOwner{
        require(_maxLimit > totalSupply().div(200),"Limit cannot be too low");
        maxKiwiLimit = _maxLimit;
    }
